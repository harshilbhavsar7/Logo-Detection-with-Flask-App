...
def store_face(image,image_name,x1,y1,x2,y2,acc,class_name):
    try :
        ...
        predictionpath=os.path.join(r'C:\Harshil\Study\pratham\Final_Project\Logo_detection_API\static',os.path.splitext(image_name)[0]+'_logo_output.jpg')
...
def store_logo(image,image_name,x1,y1,x2,y2,acc,class_name):
    try :
        output_dir="./api_output"
        folder=create_today_folder(output_dir)
        roi=image[y1:y2,x1:x2]
        predictionpath=os.path.join(r'C:\Harshil\Study\pratham\Final_Project\Logo_detection_API\static',os.path.splitext(image_name)[0]+'_'+class_name+'.jpg')
        image_name= os.path.basename(predictionpath)
        print(predictionpath)
        cv2.imwrite(predictionpath,roi)
        return image_name
