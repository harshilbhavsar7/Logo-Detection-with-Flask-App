...
from custom.storing import store_image,store_face,store_logo
...
def process_logo(image,image_name, x1, y1, x2, y2, acc, class_name):	
	stored_path= store_logo(image,image_name,x1,y1,x2,y2,acc,class_name)
	return stored_path
		...
		for i in range(len(labels)):
			...
			if row[4] >= 0.2:
				...
				if class_name not in classes_check:
					...
					# else:
					print(class_name)
					output = process_logo(image,image_name,x1, y1, x2, y2, acc, class_name)
					ocrdict['class_name']=class_name
					ocrdict['filepath'] = output
					ocrdict['Accuracy']=acc
				else:
					continue

		ocrdict.update(dict_cat)
		print(ocrdict)
		# store_logo(img,original,image,image_name,hashvalue,predicts,ocrdict)
